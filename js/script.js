...
function transformExpression(expr) {
  if (/sin\d+|cos\d+/.test(expr)) {
    return expr.replace(
      /(sin|cos)(\d+)/g,
      (match, operator, angle) => `${operator}(${angle} * Math.PI / 180)`
    );
  } else {
    return expr;
  }
...
function equal() {
  ...
  let resultTransform = transformExpression(res);
  let output = eval?.(`with (Math) ${resultTransform};`);
...
function showHistory() {
  ...
  for (let key in history) {
    log +=
      history[key].expression + " = " + history[key].result + "<br>" + "<br>";
  }
